...
import base64
import json, stripe , arrow ,re , boto3
                ...
                if customer:
                    try:
                        ...
                        if user:
                            ...
                            subject, from_email, to = 'Somebody has just registered to the Events Manager', \
                                                      'lucalicata81@gmail.com', 'lucalicata@hotmail.com'
...
# Subsription Management Section
...
def reactivate_subscription(request, pk):
    try:
        ...
        stripe.Subscription.modify(customer_sub.subscriptions.data[0].id, items=[{"id": item_id, "plan": "REG_MONTHLY"}])
...
# Events Management Application Main Section
    ...
    if request.method == 'POST':
        ...
        while i < len(data_uri_array):
            if "https" not in data_uri_array[i]:
                ...
                i += 1
        removeFiles = Timer(40.0, cleanupthumbnailsfiles)
        removeFiles.start()
        if chapters_id == "0":
            try:
                with connection.cursor() as cursor:
...
def thumbnail_upload(request):
    if request.method == 'POST':
        ...
        removeFiles = Timer(20.0, cleanupthumbnailsfiles)
        removeFiles.start()
        return HttpResponse('Your Thumbnail Has been Successfully Set')


# Site/Events Attendance Monitoring
